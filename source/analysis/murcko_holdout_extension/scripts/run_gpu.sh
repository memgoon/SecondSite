...
flock -n 9 || { echo 'Another Murcko-extension run is active'; exit 2; }
...
archive=murcko_holdout_extension_gpu_return_v1.tar.gz
tar -czf "$package/$archive.tmp" \
    analysis/murcko_holdout_extension/gpu_output/RUN_CONTRACT.json \
    analysis/murcko_holdout_extension/gpu_output/FITS_VALIDATION.json \
    analysis/murcko_holdout_extension/gpu_output/FIT_SUMMARY.tsv \
    analysis/murcko_holdout_extension/gpu_output/benchmark \
    analysis/murcko_holdout_extension/gpu_output/logs
