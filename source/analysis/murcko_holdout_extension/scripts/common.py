...
ARMS = ('every_pair',)
FILES = {'every_pair': 'EVERY_PAIR.tsv.gz'}
MODELS = ('ligand', 'protein', 'c1', 'c2', 'c3', 'd1', 'd2', 'd3')
SEEDS = (20260817, 20260818, 20260819)
VERSION = 'murcko_holdout_extension_v1'
...
def connectivity_partition(frame, fold):
...
def scaffold_set(frame):
    return {s for value in frame.murcko_scaffolds_json for s in json.loads(value) if s}


def scaffold_overlap_mask(frame, blocked):
    return frame.murcko_scaffolds_json.map(lambda value: bool(set(json.loads(value)) & blocked))


def partition(frame, fold):
    """Nested purge of the ORIGINAL connectivity-disjoint split: no reinstatement.

    Test has priority; remove test scaffolds from original validation and train,
    then retained-validation scaffolds from original train. Empty Murcko scaffolds
    are NOT one shared chemotype. Their connectivity exclusion remains enforced.
    """
    train0, val0, test, _ = connectivity_partition(frame, fold)
    blocked_test = scaffold_set(test)
    val = val0[~scaffold_overlap_mask(val0, blocked_test)].copy()
    train = train0[~scaffold_overlap_mask(train0, blocked_test | scaffold_set(val))].copy()
    used = set(train.main_row_id) | set(val.main_row_id) | set(test.main_row_id)
    excluded = frame[~frame.main_row_id.isin(used)].copy()
    for field in ('main_row_id', 'uniprot', 'family_component_id', 'connectivity_key'):
        a, b, c = [set(x[field].astype(str)) for x in (train, val, test)]
        if a & b or a & c or b & c:
            raise ValueError('Overlap: ' + field)
    a, b, c = [scaffold_set(x) for x in (train, val, test)]
    if a & b or a & c or b & c:
        raise ValueError('Nonempty Murcko scaffold overlap')
    for name, x in [('train', train), ('validation', val), ('test', test)]:
        if set(x.binary_label.astype(int)) != {0, 1}:
            raise ValueError('Empty or single-class partition: ' + name)
    assert len(used) + len(excluded) == len(frame)
    assert set(train.main_row_id) <= set(train0.main_row_id)
    assert set(val.main_row_id) <= set(val0.main_row_id)
    return train, val, test, excluded


def read_frame(arm):
    return pd.read_csv(PACKAGE / 'data' / FILES[arm], sep='\t',
                       dtype={'main_row_id': str}, keep_default_na=False)
...
def fit_dir(arm, model, seed, fold):
    return PACKAGE / 'gpu_output/benchmark/fits' / arm / 'pfam_murcko' / model / ('seed_%s' % seed) / ('fold_%s' % fold)
