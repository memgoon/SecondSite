#!/usr/bin/env python3
"""Vendored matrix fit loop; explicit splits and separately fingerprinted contracts.
Source changes are documented in README.md. Not a standalone entrypoint.
"""
...
def load_base_trainer(root: Path):
    path = Path(__file__).resolve().parent / "base_trainer.py"
...
def checkpoint_selection(validation_metrics, regime, model_name):
    ...
    """
    if regime == "pfam_murcko":
        regime = "unseen_family"  # Preserve selection rule of the matched family baseline.
...
def split_frame(frame, arm, regime, test_fold, weight_function):
    ...
    train, validation, test, discarded = partition(frame, test_fold)
    train["training_weight"] = weight_function(train)
    validation["training_weight"] = 1.0
    test["training_weight"] = 1.0
    test["unseen_compound"] = 1
    return train, validation, test, (int(test_fold) + 1) % 5
...
def completed(paths, expected_ids, contract, identity):
    if not paths["report"].is_file() or not paths["prediction"].is_file() or not paths["checkpoint"].is_file() or not paths["history"].is_file():
        return False
    try:
        ...
        return (
            ...
            and report.get("history_sha256") == sha256(paths["history"])
        )
    except Exception:
        return False
...
def fit_one(
    ...
    contract = dict(cpu_contract["training_contract"])
    contract.update({
        "run_fingerprint": cpu_contract["run_fingerprint"],
        "split_sha256": cpu_contract["split_sha256"],
        "cohort_sha256": cpu_contract["cohort_sha256"][arm],
    })
