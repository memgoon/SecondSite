...
from common import verify_contract, partition, read_frame, expected_jobs, auc, ap, write_json, resample_family_statistics
...
def main():
    ...
    assert all_folds[ARMS[0]] == all_folds[ARMS[1]]
    assert len(expected_jobs()) == len(set(expected_jobs())) == c['expected_fits'] == 240
    ...
    from aggregate import contrast_job, stats
    ...
    for model in MODELS:
        for protein_groups, ligand_groups in [(20, 20), (20, 3), (3, 20), (3, 3)]:
            ...
            assert select(metrics, 'double_unseen', model) == select(metrics, 'unseen_family', model)
    write_json(PACKAGE / 'validation/STATIC_VALIDATION.json', dict(status='validated',
        source_hashes_match=True, fits=240, explicit_partitions_match_frozen_manifest=True,
        ...
        bootstrap_multiplicity_self_test=True, gpu_forward_and_training='not run on this CPU host'))
