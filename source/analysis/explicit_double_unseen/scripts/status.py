"""Lightweight progress: inspect only the 240 expected FIT_REPORT paths."""
import json
from collections import Counter, defaultdict
from common import PACKAGE, expected_jobs, fit_dir
...
for arm, model, seed, fold in expected_jobs():
    ...
    try:
        ...
        pass
print('Completed reports: %d/240 (lightweight count, not full hash validation)' % sum(counts.values()))
for arm in ('every_pair', 'protein_anchored'):
    print('%s: %d/120' % (arm, counts[arm]))
for model, values in sorted(times.items()):
    print('%s: %d/30 fits, observed mean %.1f min/fit' % (model, len(values), sum(values)/len(values)/60))
