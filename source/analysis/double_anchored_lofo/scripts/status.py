"""Current run progress from 2,256 known report paths, not historical logs."""
import json
import time
...
def main():
    ...
    if not path.exists():
        print('No run started. Expected 2,256 fits.')
        return
    ...
    for job in jobs():
        if compatible_report(job,run,hashes=False):
            counts[job[0]]+=1
            total+=1
    print('Completed current-run fits: %d / 2256 (%.1f%%)'%(total,100.*total/2256))
    print(dict(counts))
