...
def main():
    ...
    if args.mode=='input':
        archive=PACKAGE/'ligand_anchored_benchmark_gpu_input_v1.tar.gz'
    ...
    for job in jobs():
        allowed.update(str((fit_dir(job)/p).relative_to(PACKAGE)) for p in ('FIT_REPORT.json','best.pt','predictions.tsv.gz','validation_predictions.tsv.gz','history.tsv'))
