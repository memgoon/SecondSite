"""Read only 240 expected fit paths; independently validate returned artifacts."""
import json
import numpy as np
import pandas as pd
from common import PACKAGE, ARMS, expected_jobs, fit_dir, sha, verify_contract, read_frame, partition, write_json
...
def collect():
    ...
    for a, model, seed, fold in expected_jobs():
        ...
        identity = dict(cohort_arm=a, model=model, seed=seed, outer_fold=fold,
                        validation_fold=(fold + 1) % 5, regime='double_unseen')
    ...
    assert len(predictions) == 2 * 8 * 3 * 4637
    assert sum(x['checkpoint_selection_fallback_at_best_epoch'] for x in reports) == c['expected_fallback_fits']
    write_json(PACKAGE / 'gpu_output/FITS_VALIDATION.json', dict(status='validated', observed_fits=240,
...
if __name__ == '__main__':
    collect()
    print('PASS: all 240 fits and prediction/checkpoint hashes validated.')
